...
let watering_unm = 1;
let status = [11,12,13]
let tid = status[Math.floor(Math.random()*status.length)];
///////////////////////////////////////////////////////////////////
...
async function sun() {
    let options = {
        method: 'POST',
        url: `${hostname}/api/gainTaskAward?taskid=${tid}`,
        headers: {
            'Host': host,
            'autohrization': `${token}`
        },
    ...
    let result = await httpRequest(options, `收集阳光`);

     if (result.data.errcode == 19) {
        DoubleLog(`\n 收集阳光: 任务已完成`);
     }else if (result.status == 50014) {
        DoubleLog(`\n ${result.message}`);
     } else if (result.data.gainaward.type == 1) {
        DoubleLog(`\n收集阳光: 获得${result.data.gainaward.value}`);
    }
}
